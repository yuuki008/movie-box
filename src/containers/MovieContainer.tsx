...
import { API_GET_MOVIE_NOW_PLAYING, API_GET_MOVIE_POPULAR, API_GET_MOVIE_TOP_RATED, API_GET_MOVIE_UPCOMING } from '../api'
...
const MovieContainer = () => {
    ...
    return (
        <div style={{
            ...
            paddingTop: '60px'}}>
            <h2
                style={{position: 'fixed', top: 90, left: 80}}
            >{pageTitle(path)}</h2>
            {!keyword ? (
            ...
            {!isFetching ? (
                movies !== undefined || movies.length > 0 ? (
                    <>
                    <Div>
                        <div style={{overflowX: 'scroll', display: 'flex', alignContent: 'center'}}>
                            {movies.map((movie: any) => 
                                <DefaultCard info key={movie.id} id={movie.id} path={movie.poster_path} title={movie.title} name={movie.name} voteAverage={movie.vote_average} release_date={movie.release_date} voteCount={movie.vote_count} first_air_date={movie.first_air_date}/>
                            )}
                        </div>
                    </Div>
                ...
                <h2 style={{textAlign: "center", marginTop: "50px"}}></h2>
            )}
        </div>
    )
}
